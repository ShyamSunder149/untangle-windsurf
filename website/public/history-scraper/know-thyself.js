...
const USER_ID = 'eb978738-57fa-478e-9ab7-616040830f4f'; // Sample user UUID
...
async function main() {
  ...
  // const flowchart = await fetchGeminiFlowchart(); // Bypass fetch
  const flowchart = HARDCODED_MERMAID_CHART; // Use hardcoded chart directly
  showStatus('Using pre-defined visualization.');
  const journeyMapDiv = document.getElementById('journeyMap');
  journeyMapDiv.innerHTML = `<pre class='mermaid'>${flowchart}</pre>`;

  // Ensure mermaid is available and initialize
  if (window.mermaid) {
    try {
      // Use the new API if available, otherwise fallback
      if (typeof mermaid.initialize === 'function') {
        mermaid.initialize({ startOnLoad: false }); // Don't auto-render
        await mermaid.run({ nodes: journeyMapDiv.querySelectorAll('.mermaid') });
        showStatus('Visualization rendered!');
      } else if (typeof mermaid.init === 'function') {
        // Fallback for older Mermaid versions
        mermaid.init(undefined, journeyMapDiv.querySelectorAll('.mermaid'));
        showStatus('Visualization rendered (legacy)!');
      }
    } catch (error) {
      console.error("Mermaid rendering failed:", error);
      showStatus('Failed to render visualization.');
      journeyMapDiv.textContent = 'Error rendering Mermaid diagram. Check console.';
    }
  } else {
    console.error('Mermaid library not loaded.');
    showStatus('Mermaid library not found.');
    journeyMapDiv.textContent = 'Mermaid library failed to load. Cannot display diagram.';
  }
}
